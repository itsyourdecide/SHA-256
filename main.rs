...
impl Sha256 {
    ...
    /// PHASE 3: Compression function
    fn process_block(&mut self, chunk: &[u8]) {
        // PHASE 2: Message schedule preparation and expansion
        ...
        // Initialization of working registers
        let mut vars = self.h; // Copy current state into [a, b, c, d, e, f, g, h]

        // MAIN COMPRESSION LOOP
        for i in 0..64 {
            let t1 = vars[7] // h
                .wrapping_add(Self::sigma_upper_1(vars[4])) // e
                .wrapping_add(Self::ch(vars[4], vars[5], vars[6])) // e, f, g
                .wrapping_add(Self::K[i])
                .wrapping_add(self.w[i]);
            
            let t2 = Self::sigma_upper_0(vars[0]) // a
                .wrapping_add(Self::maj(vars[0], vars[1], vars[2])); // a, b, c

            // Cascading state shift
            vars[7] = vars[6]; // h = g
            vars[6] = vars[5]; // g = f
            vars[5] = vars[4]; // f = e
        ...
        // PHASE 4: Davies-Meyer Feedforward (Idiomatic implementation)
        for (h_i, v_i) in self.h.iter_mut().zip(vars.iter()) {
            *h_i = h_i.wrapping_add(*v_i);
        }
    }

    /// Public interface for obtaining the final hash
    pub fn finalize(mut self, input: &[u8]) -> String {
...
fn main() {
    ...
    let result = hasher.finalize(input);
